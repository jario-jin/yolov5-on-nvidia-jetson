...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        std::ofstream ofs("annotations/" + file_names[f - fcount + 1 + b] + ".json");
        // for (int b = 0; b < fcount; b++) {
        b = 0;
        auto& res = batch_res[b];
        // std::cout << res.size() << std::endl;
        cv::Mat img = cv::imread(img_dir + "/" + file_names[f - fcount + 1 + b]);
        ofs << "{\"infer_time\":" << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << ",\"file_name\":\"" << file_names[f - fcount + 1 + b] << "\",\"height\":" << img.rows << ",\"width\":" << img.cols << ",\"annos\":[";
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            ofs << "{\"score\":" << res[j].conf << ",\"area\":" << r.width*r.height << ",\"bbox\":[" << r.x << "," << r.y << "," << r.width << "," << r.height << "],\"category_name\":\"" << (int)res[j].class_id << "\"}";
